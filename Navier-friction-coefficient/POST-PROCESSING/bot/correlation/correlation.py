...
def main(k):
    in_fname = '../../../LAMMPS/r' + str(int(k+1)) + '_bot.dat'   # Bottom wall LAMMPS data
    ...
    np.savetxt(cuu_out_fname, np.c_[np.arange(len(cuu))*dt,\
            cuu, cuu_norm], fmt='%.10f \t %.20f  \t %.20f')  
    np.savetxt(cuf_out_fname, np.c_[np.arange(len(cuf))*dt,\
            cuf, cuf_norm], fmt='%.10f \t %.20f \t %.20f') 
    return cuu, cuf
